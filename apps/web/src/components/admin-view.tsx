...
export function AdminView() {
  ...
  const [csvText, setCsvText] = useState("rollNumber,password,displayName,sectionId,wingId,cr,wingPocWings,subjectPocOfferings\n");
  ...
  async function commitRoster() {
    setBusy(true); setError(null);
    try { const result = await callFunction<unknown, { total: number }>("commitRosterImport", { rows: parseCsv(csvText) }); setMessage(`${result.total} roster records committed.`); setPreview(null); setCsvText("rollNumber,password,displayName,sectionId,wingId,cr,wingPocWings,subjectPocOfferings\n"); }
